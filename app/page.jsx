export default function RomanaHairLandingPage() {
  ...
  return (
    <main className="min-h-screen bg-black text-white selection:bg-[#c9a34d] selection:text-black">
      ...
      <header className="fixed top-0 left-0 right-0 z-50 bg-black/35 backdrop-blur-xl border-b border-white/5">
        <div className="max-w-[1360px] mx-auto px-5 lg:px-10 h-24 flex items-center justify-between">
          ...
          <nav className="hidden lg:flex items-center gap-9 text-[13px] uppercase tracking-[0.12em] font-semibold text-white/85">
            ...
            <a className="hover:text-[#d6b35f] transition" href="#profissionais">Profissionais</a>
        ...
        <div className="max-w-[1360px] mx-auto">
          ...
          <div className="grid lg:grid-cols-4 gap-6 mb-16">
            {showcase.map((image, index) => (
              <div key={image} className={`${index === 0 ? 'lg:col-span-2 lg:row-span-2' : ''} group relative min-h-[420px] lg:min-h-[520px] rounded-[28px] overflow-hidden border border-white/10 bg-zinc-900 shadow-2xl`}>
      ...
      <section id="profissionais" className="relative z-10 px-5 lg:px-10 py-24 bg-black">
        <div className="max-w-[1360px] mx-auto">
          <div className="grid lg:grid-cols-[0.78fr_1.22fr] gap-10 items-end mb-14">
            <div>
              <p className="text-[#d6b35f] uppercase tracking-[0.32em] text-sm font-bold mb-4">Nossos Profissionais</p>
              <h2 className="font-serif text-4xl lg:text-6xl leading-tight">
                Conheça os trabalhos
                <span className="block italic text-[#d6b35f]">de cada especialista.</span>
              </h2>
            </div>
            <p className="text-white/70 text-lg leading-relaxed max-w-2xl lg:ml-auto">
              Clique no profissional para abrir uma galeria com resultados reais, estilos e detalhes dos atendimentos feitos no Romana Hair Studio.
            </p>
          </div>

          <div className="space-y-7">
            {professionals.map((professional, index) => (
              <details key={professional.name} open={index === 0} className="group rounded-[30px] border border-white/10 bg-[#0d0d0d] shadow-[0_25px_80px_rgba(0,0,0,0.45)] overflow-hidden open:border-[#d6b35f]/65">
                <summary className="grid cursor-pointer list-none lg:grid-cols-[260px_1fr_auto] gap-6 items-center p-5 lg:p-7 [&::-webkit-details-marker]:hidden">
                  <div className="aspect-[3/4] rounded-[24px] overflow-hidden bg-[#050505]">
                    <img src={professional.cover} alt={`Trabalho de ${professional.name}`} className="h-full w-full object-contain transition duration-700" />
                  </div>
                  <div>
                    <p className="text-[#d6b35f] uppercase tracking-[0.22em] text-xs font-black mb-3">{professional.role}</p>
                    <h3 className="font-serif text-4xl lg:text-5xl mb-4">{professional.name}</h3>
                    <p className="text-white/68 leading-relaxed max-w-2xl">{professional.description}</p>
                  </div>
                  <span className="inline-flex h-14 w-14 items-center justify-center rounded-full border border-[#d6b35f] text-[#d6b35f] text-3xl leading-none transition group-open:rotate-45 group-open:bg-[#d6b35f] group-open:text-black">+</span>
                </summary>
                ...
                <div className="border-t border-white/10 px-5 pb-7 lg:px-7 lg:pb-8">
                  ...
                  <div className="mt-7">
                    <a href={whatsapp} target="_blank" rel="noreferrer" className="inline-flex rounded-xl bg-gradient-to-r from-[#b98b32] via-[#e6c977] to-[#b98b32] px-7 py-4 text-black font-extrabold uppercase tracking-wide hover:scale-[1.02] transition">
